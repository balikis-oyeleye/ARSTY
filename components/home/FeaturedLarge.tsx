        ...
        return (
          <div key={product.id} className={`md:flex border-t-[0.5px] pt-8 mb-8 ${index === 1 ? 'flex-row-reverse' : 'flex-row'} gap-x-[37px]`}>
            ...
            <div className='flex flex-col gap-y-[38px] h-full flex-1'>
              <h3 className='font-stix text-2xl lg:text-[40px] text-grey font-bold hidden md:block'>{product.title}</h3>
              <p className='lg:text-2xl text-base font-normal text-grey font-satoshi'>{product.text}</p>
              <div className='flex items-center justify-between'>
                <Creators />
                <p className='font-satoshi font-medium lg:text-2xl text-base text-grey '>64 major creators</p>
                <div className='text-grey2 border border-grey2 p-2 rounded-full hidden md:block'><BsArrowRight size={24} /></div>
              </div>
            </div>
          </div>
        )
